...
I audited the public main branch and live deployment: , , and live app.
### Phase 1: Deep technical audit
#### Release blockers and architecture
1. Dependency manifest is out of syncSeverity: CriticalFiles: ,
...
2. Audit schema import is broken and there are two competing audit modelsSeverity: CriticalFiles: , , ,
...
3. Inngest events are sent, but no worker route was foundSeverity: CriticalFiles: ,
...
4. The public landing page is a 75 KB client componentSeverity: HighFile:
...
```
5. The public audit is simulated, not connected to the backendSeverity: HighFiles: ,
...
6. Dashboard protection is client-side onlySeverity: HighFiles: , ,
...
7. Redirects lose the locale and can produce 404sSeverity: MediumFile:
...
8. Registration collects a workspace name and discards itSeverity: HighFile:
...
9. OAuth and password recovery are placeholdersSeverity: HighFiles: ,
...
10. Local storage parsing can crash the dashboard shellSeverity: MediumFile:
...
11. Login and registration accept any credentialsSeverity: CriticalFile:
...
12. API tenant headers are spoofableSeverity: CriticalFiles: , ,
...
13. Premium audit has no subscription or role checkSeverity: HighFile:
...
```
14. Public and premium audit routes lack abuse protectionSeverity: HighFiles: ,
...
```
15. URL validation is not enough to prevent SSRFSeverity: HighFiles: ,
...
Fix: reuse for every crawl entry point, including Firecrawl-backed routes. Enforce HTTP and HTTPS only, resolve DNS before connecting, reject private ranges, and validate every redirect target.
16. Premium crawling and LLM analysis run inside the HTTP requestSeverity: HighFile:
...
17. Database errors are swallowed and converted into false successSeverity: CriticalFile:
...
18. Transaction state is global and unsafe under concurrencySeverity: CriticalFile:
...
```
19. Dashboard statistics load every audit into memorySeverity: HighFile:
...
20. RLS and application-level tenant filtering are inconsistentSeverity: HighFiles: , ,
...
21. Asset payload is heavySeverity: HighFiles: ,
...
22. Global client providers are mounted on every public routeSeverity: MediumFile:
...
23. Animation and graph work is not isolated from the first paint****Severity: MediumFiles include , , and .
...
24. Dashboard refresh architecture is inconsistent****Severity: MediumFile:
...
25. Metadata is static and not properly localized****Severity: HighFile:
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: isFa
      ? "سئورچبل، پایش دیدهشدن برند در موتورهای هوش مصنوعی"
      : "Seorchable, AI Brand Visibility Intelligence",
    description: isFa
      ? "پایش و بهینهسازی حضور برند در ChatGPT، Gemini، Claude و Perplexity."
...
26. Robots, sitemap, JSON-LD, and route SEO files are missingSeverity: HighFiles: ,
...
27. Locale validation is missing****Severity: MediumFile:
...
28. Accessibility needs a dedicated pass****Severity: MediumFiles include and .
