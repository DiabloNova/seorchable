    ...
    <div className="space-y-6 animate-fade-in text-start">
      <div>
        <h1 className="text-2xl font-black text-[var(--text-primary)] font-display flex items-center gap-2.5">
          ...
          <span>{isRtl ? "کاوشگر استنادها" : "Citation Explorer"}</span>
        ...
        <p className="text-xs text-[var(--text-secondary)] mt-1.5 max-w-2xl">
          {isRtl
            ? "پایش و تحلیل لینک‌های ارجاع داده شده از سمت موتورهای جستجوی هوشمند به دامنه‌های شما."
            : "Monitor and verify citation backlink profiles, domain authority metrics, and citation integrity."}
        </p>
      </div>
      ...
      <Card className="border border-[var(--border)] bg-[var(--card)]">
        ...
        <CardContent className="text-xs text-[var(--text-secondary)] leading-relaxed space-y-2">
          <p>
            {isRtl
              ? "مسیر فعال سایدبار: ابزارهای برند و استناد > کاوشگر استنادها"
              : "Active Sidebar Navigation State: Brand & Citation Tools > Citation Explorer"}
          </p>
