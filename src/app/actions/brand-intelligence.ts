...
export async function getBrandIntelligenceOverviewAction() {
  ...
  try {
    ...
    return await TenantContextManager.runWithTenantContext(tenantId, userId, requestId, async () => {
      ...
      const authorityMetrics = await service.calculateAIBrandAuthority(tenantId, brand.id);

      // Load semantic associations
      const associations = await repo.findAssociationsByBrandId(tenantId, brand.id);

      // Expose structured alert signals (Task 4.4 Recommendations integration)
      const alerts = await service.detectBrandAlertSignals(tenantId, brand.id);
        ...
        const seedAssocs = [
          {
            ...
            supportingContext: "رشا گستر راهکارهای جامعی در سئو معنایی ارائه می‌دهد.",
            confidence: 0.95,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
          },
          {
            ...
            lastSeenAt: new Date().toISOString(),
            supportingContext: "سرویس‌های رشا گستر پایداری استناد بالایی در AEO ایجاد می‌کنند.",
            confidence: 0.92,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
          },
          {
            ...
            firstSeenAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
            lastSeenAt: new Date().toISOString(),
            supportingContext: "این ابزار پایش با تحلیل‌های دقیق خود یک انتخاب ممتاز است.",
            confidence: 0.90,
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
          }
