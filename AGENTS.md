...
Sections §1 to §22 define what correct work looks like. This section defines how you continuously verify that you are still doing it.

### 23.2 The alignment contract
Every action you take must be simultaneously compatible with all six invariants below:
1. Project objective
2. Task objective
3. Task scope
...
If you cannot answer all six, you are diverging. Stop.

### 23.3 Pre-execution alignment gate
Before the first edit of any task, produce an Alignment Record and place it in the task report. Fewer than six lines is not an Alignment Record. No edit may precede it.
1. Objective
2. Non-objectives
3. Scope
4. Preconditions
5. Boundaries touched
6. Verification plan
Gate failed = do not start.

### 23.5 Divergence triggers
Stop immediately if any of these occur:
A1. Conflicts with project objective
A2. Conflicts with task objective
A3. Exceeds task scope (editing unapproved files)
A4. Introduces unrelated changes
A5. Bypasses architecture
A6. Weakens security
A7. Violates tenant isolation

### 23.8 Final scope-alignment check
Before declaring completion, answer F1 to F8 explicitly in your report against the actual diff:
